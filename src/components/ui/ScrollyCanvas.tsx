...
export default function ScrollyCanvas({
  ...
  const renderFrame = useCallback(
    (index: number) => {
      ...
      // Draw Logic (Contain - show full image)
  ...
  const getFrameIndex = useCallback(
    (progress: number) => {
      ...
      if (progress <= splitPoint) {
        ...
        frameIndex =
          splitFrame +
          ((progress - splitPoint) / (1 - splitPoint)) *
            (totalFrames - splitFrame);
      }
  ...
  useEffect(() => {
    ...
    const loadSequence = async () => {
      ...
      if (isMounted) {
        ...
        if (!isMounted) return;
        chunk3.forEach((img, idx) => {
          imagesRef.current[80 + idx] = img;
        });
        updateCurrentView();
      }
      ...
      if (isMounted) {
        ...
        if (!isMounted) return;
        chunk4.forEach((img, idx) => {
          imagesRef.current[120 + idx] = img;
        });
        updateCurrentView();
      }
    };
